...
// dht11on : DHT11_processing 내부실행
// dht11off: DHT11_processing 실행 안함
// dht11status 를 입력하면 dht11on과 dht11time 출력
int dht11on =1;	// dht11on : 1 DHT11_processing를 실행하고 dht11on : 0 return
extern int dht11time;		// pc 로 부터 dht11 check time을 읽어서 ascii to int 저장하는 변수
void DHT11_processing(void)
{
	...
	if (TIM11_10ms_counter >= dht11time) // 1500ms
	{
		...
		d_RH = DHT11_rx_Data();  // 습도 소숫점
		i_Tmp = DHT11_rx_Data();  // 온도 정수
		d_Tmp = DHT11_rx_Data();  // 온도 소숫점
